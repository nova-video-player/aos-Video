...
public class VideoPreferencesCommon implements OnSharedPreferenceChangeListener {
    ...
    public static final String KEY_AUDIO_SPEED_MODE = "audio_speed_mode";
    public static final String AUDIO_SPEED_MODE_DISABLED = "0";
    public static final String AUDIO_SPEED_MODE_AUDIOTRACK = "1";
    public static final String AUDIO_SPEED_MODE_ATEMPO = "2";
    public static final String AUDIO_SPEED_MODE_SONIC = "3";
    ...
    private ListPreference mAudioSpeedMode = null;
    private CheckBoxPreference mEnableDynamicAudioDelay = null;
    private ListPreference mEnableSpatialization = null;
    ...
    /** Software modes remain selectable without platform support; encoded output bypasses all modes. */
    private void updateSpatializationPreferenceState(boolean passthroughEnabled) {
        if (mEnableSpatialization == null) {
            return;
        }
        boolean toggleAvailable = !passthroughEnabled;
     ...
     * or an active spatialization mode both make these preferences moot
    ...
    public static boolean isAudioSpeedEnabled(SharedPreferences preferences) {
        ...
        if (AUDIO_SPEED_MODE_DISABLED.equals(preferences.getString(KEY_AUDIO_SPEED_MODE, AUDIO_SPEED_MODE_ATEMPO))) return false;
        try {
    ...
    /**
     * Update the enabled and selectable state of the audio speed mode preference
     * based on passthrough settings and software audio decoder choice. When passthrough
     * is enabled or MediaCodec is used as the audio decoder (instead of FFmpeg), audio
     * speed controls are disabled, greyed out and skipeable.
     *
     * @param passthroughEnabled true if audio passthrough is enabled
     */
    private void updateAudioSpeedState(boolean passthroughEnabled) {
        ...
        if (mAudioSpeedMode != null) {
            mAudioSpeedMode.setEnabled(speedAllowed);
            mAudioSpeedMode.setSelectable(speedAllowed);
        }
    }
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        mAudioSpeedMode = (ListPreference) findPreference(KEY_AUDIO_SPEED_MODE);
        mEnableDynamicAudioDelay = (CheckBoxPreference) findPreference(KEY_ENABLE_DYNAMIC_AUDIO_DELAY);
        mEnableSpatialization = (ListPreference) findPreference(SpatializationSettings.KEY_MODE);
        mEnableSpatialization.setValue(Integer.toString(SpatializationSettings.getMode(mSharedPreferences)));
        mEnableSpatialization.setSummaryProvider(ListPreference.SimpleSummaryProvider.getInstance());
        ...
        final boolean isSpatializationSupportedByPlatform = SpatializationSettings.isSystemAvailable();
        mEnableSpatialization.setOnPreferenceChangeListener((preference, newValue) -> {
            int mode = Integer.parseInt((String) newValue);
            boolean spatial = mode >= SpatializationSettings.TV ||
                    (mode == SpatializationSettings.SYSTEM && SpatializationSettings.isSystemAvailable());
            updateDownmixPreferencesState(SpatializationSettings.isPassthrough(mSharedPreferences) || spatial);
            return true;
        });
        ...
        boolean spatializationOverridesDownmix = SpatializationSettings.effectiveMode(mSharedPreferences) != SpatializationSettings.OFF;
        if (isPassthroughSupported) {
            ...
            updateSpatializationPreferenceState(passthroughEnabled);
            ...
            mForceAudioPassthroughMultiple.setOnPreferenceChangeListener((preference, newValue) -> {
                ...
                updateSpatializationPreferenceState(newPassthroughEnabled);
                updateDownmixPreferencesState(newPassthroughEnabled
                        || (Integer.parseInt(mEnableSpatialization.getValue()) >= SpatializationSettings.TV
                        || (isSpatializationSupportedByPlatform && "1".equals(mEnableSpatialization.getValue()))));
                return true;
            });
        } else {
            ...
            updateSpatializationPreferenceState(false);
