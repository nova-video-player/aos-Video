...
public class BrowserAllMovies extends CursorBrowserByVideo {
	...
	static final public String LIST_OF_MOVIES_IDS = "ListOfMovieIds";
	...
	public String getCursorSelection() {
	    Bundle args = getArguments();
	    if (args != null) {
	        String listOfMoviesIds = args.getString(BrowserAllMovies.LIST_OF_MOVIES_IDS);
	        if (listOfMoviesIds != null) {
				return VideoColumns.SCRAPER_MOVIE_ID + " IN (" + listOfMoviesIds + ")";
	        }
	    }

	    // default is all
        return mHideWatched ? (SELECTION_ALL_MOVIES + HIDE_WATCHED_FILTER) : );
	}
	*/
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args2) {
		if(getArguments()!=null){
			String listOfMoviesIds = getArguments().getString(BrowserAllMovies.LIST_OF_MOVIES_IDS);
			if (listOfMoviesIds != null)
